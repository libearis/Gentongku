...
public sealed class CatalogDummyDataGenerator : ICatalogDummyDataGenerator
{
    ...
    public CatalogDummyDataGenerator(CatalogDbContext db) => _db = db;
    ...
    {
        var categoryIds = await _db.Categories.AsNoTracking().Select(c => c.Id).ToListAsync(ct);
        ...
        var sellerIds = await _db.SellerProfiles.AsNoTracking().Select(s => s.UserId).ToListAsync(ct);
        if (sellerIds.Count == 0)
        {
            if (!await _db.SellerProfiles.AnyAsync(s => s.UserId == CatalogSeeder.SystemSellerId, ct))
            {
                _db.SellerProfiles.Add(SellerProfile.Create(CatalogSeeder.SystemSellerId, "System Seller"));
                await _db.SaveChangesAsync(ct);
            }
